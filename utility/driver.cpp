/*
 * Modified versior of driver.c from ULAFF PfHP
 */
...
int main(int argc, char *argv[])
{
  ...
  std::printf( "# number of repeats:" );
  std::scanf( "%d", &nrepeats );
  std::printf( "# %d\n", nrepeats );
  ...
  /* Timing trials for matrix sizes m=n=k=first to last in increments
     ...
     timings.  */
  std::printf( "# enter first, last, inc:" );
  std::scanf( "%d%d%d", &first, &last, &inc );
  ...
  std::printf( "#             reference      |         current implementation \n" );
  std::printf( "n       time       GFLOPS        time       GFLOPS     diff \n" );
  i = 1;
